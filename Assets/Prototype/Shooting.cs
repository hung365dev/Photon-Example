...
public class Shooting : Photon.MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (Input.GetMouseButton(0) && Time.time > m_lastshoot)
        {
            ...
            this.photonView.RPC("ShootEffect", PhotonTargets.All);
    ...
    void OnRaycastHit(RaycastHit hit)
    {
        ...
        if (m_healthHandler.CurrentTeam != PhotonNetwork.player.GetPlayerTeam()
        || m_healthHandler.CurrentTeam == Teams.Team.aggressive)
        {
            m_healthHandler.OnReceiveDamage(m_damage, PhotonNetwork.player);
        }
